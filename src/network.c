...
{
#ifdef GPU
    if(gpu_index >= 0) return train_network_datum_gpu(net, x, y, ocr_label);
#endif
...
float train_network_sgd(network net, data d, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float err = train_network_datum(net, X, y, ocr_label);
        sum += err;
    }
    free(X);
    free(y);
