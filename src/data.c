...
int* fill_ocr_labels (const char* label_name, int *label_length) {
    ...
    int *ocr_labels = (int*) xcalloc(count, sizeof(int));
    memcpy(ocr_labels, labels, count);
    fclose(file);
    *label_length = count;
    return ocr_labels;
}
...
ocr_label_desc load_ocr_labels(char **paths, int n)
{
    ocr_label_desc ocr_label;
    ocr_label.label_length = (int*)xcalloc(n, sizeof(int));
    ocr_label.labels = (int**)xcalloc(n, sizeof(int*));
    for(int i = 0; i < n; ++i) {
        int count = 0;
        ocr_label.labels[i] = fill_ocr_labels(paths[i], &count);
        ocr_label.label_length[i] = count;
    }
    return ocr_label;
}
        ...
        for(index_j=y_min;index_j<y_max;++index_j){
            for(index_i=x_min;index_i<x_max;++index_i){
                ...
                if(class_label >1){
                    ...
                    error("class label > 1.0", DARKNET_LOC);
                }
...
data load_data_ocr(int n, char **paths, int m, int w, int h) {
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
    d.X = load_image_paths(paths, n, w, h);
    d.ocr_label = load_ocr_labels(paths, n);
    if(m) free(paths);
    return d;
}

void convertToPoly(float* val, PolyGon_S* polygon_label) {
    polygon_label[0].label_id = val[0];
    polygon_label[0].leftTop.x = val[1];
    polygon_label[0].leftTop.y = val[2]; 
    polygon_label[0].rightTop.x =val[3]; 
    polygon_label[0].rightTop.y =val[4]; 
    polygon_label[0].rightBottom.x = val[5];
    polygon_label[0].rightBottom.y = val[6]; 
    polygon_label[0].leftBottom.x = val[7]; 
    polygon_label[0].leftBottom.y = val[8]; 

    polygon_label[1].label_id = val[9];
    polygon_label[1].leftTop.x = val[10];
    polygon_label[1].leftTop.y = val[11];
    polygon_label[1].rightTop.x = val[12];
    polygon_label[1].rightTop.y = val[13];
    polygon_label[1].rightBottom.x = val[14];
...
void convertFromPoly(float* val, PolyGon_S polygon_label) {
    val[0] = polygon_label.label_id;
    val[1] = polygon_label.leftTop.x;
    val[2] = polygon_label.leftTop.y;
    val[3] = polygon_label.rightTop.x;
    val[4] = polygon_label.rightTop.y;
    val[5] = polygon_label.rightBottom.x;
    val[6] = polygon_label.rightBottom.y;
    val[7] = polygon_label.leftBottom.x;
    val[8] = polygon_label.leftBottom.y; 
}

matrix load_PolyGon_Points(char **paths, int n)
{
    matrix y = make_matrix(n, 18);
    for(int i = 0; i < n; ++i) {
        int count = 0;
...
data load_data_homepage(int n, char **paths, int m, int w, int h) {
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
    d.X = load_image_paths(paths, n, w, h);
    d.y = load_PolyGon_Points(paths, n);
    if(m) free(paths);
    return d;
}
